...
"use strict";
import slugify from "slug-arabic";
...
class SequelizeSlugify {
  slugifyModel(model, options) {
    ...
    const handleSlugify = async function (
      instance,
      sequelizeOptions,
      forceGenerateSlug
    ) {
      ...
      if (
        slugValue &&
        slugifyOptions.overwrite === false &&
        forceGenerateSlug !== true
      ) {
        return instance;
      }
      ...
      const transaction =
        slugifyOptions.passTransaction === true
      ...
      ({ suffixedSlug: slugValue, isUnique } = await addSourceSuffix(
        instance,
        slugValue,
        slugifyOptions.suffixSource,
        transaction
      ));
    ...
    const slugifyFields = function (instance, sourceFields) {
      ...
      const options =
        (slugifyOptions && slugifyOptions.slugOptions) || DEFAULT_SLUG_OPTIONS;
      const slug = slugify(slugParts.join(" "), options);
      return slug;
    };
    ...
    const addSourceSuffix = async function (
      instance,
      slugValue,
      suffixFields,
      transaction
